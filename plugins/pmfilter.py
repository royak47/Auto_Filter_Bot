...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    use_buttons = settings.get("button", True)  # fallback True
    ...
    # Filter Buttons (common in both cases)
    btn.insert(0, [
        InlineKeyboardButton(f'Qᴜᴀʟɪᴛʏ', callback_data=f"qualities#{key}"),
        InlineKeyboardButton("Lᴀɴɢᴜᴀɢᴇ", callback_data=f"languages#{key}"),
        InlineKeyboardButton("Sᴇᴀsᴏɴ",  callback_data=f"seasons#{key}")
    ...
    if offset != "":
        try:
            page_count = math.ceil(int(total_results) / (10 if settings.get('max_btn') else int(MAX_B_TN)))
            btn.append([
                InlineKeyboardButton("ᴘᴀɢᴇ", callback_data="pages"),
                InlineKeyboardButton(text=f"1/{page_count}", callback_data="pages"),
                InlineKeyboardButton(text="ɴᴇxᴛ ⋟", callback_data=f"next_{req}_{key}_{offset}")
            ])
        except KeyError:
            await save_group_settings(chat_id, 'max_btn', True)
            page_count = math.ceil(int(total_results) / 10)
            btn.append([
                InlineKeyboardButton("ᴘᴀɢᴇ", callback_data="pages"),
                InlineKeyboardButton(text=f"1/{page_count}", callback_data="pages"),
                InlineKeyboardButton(text="ɴᴇxᴛ ⋟", callback_data=f"next_{req}_{key}_{offset}")
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('MOVIES TELECAST 📜', url='https://t.me/moviestelecast'),
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ⇋', callback_data='about')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if settings.get('button'):
            cap = f"<b>🏷 ᴛɪᴛʟᴇ : <code>{search}</code>\n🧱 ᴛᴏᴛᴀʟ ꜰɪʟᴇꜱ : <code>{total_results}</code>\n⏰ ʀᴇsᴜʟᴛ ɪɴ : <code>{remaining_seconds} Sᴇᴄᴏɴᴅs</code>\n\n📝 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {message.from_user.mention}\n ᴘᴏᴡᴇʀᴇᴅ ʙʏ : ⚡ {message.chat.title or temp.B_LINK or 'MOVIESHUB'} \n\n🧾 <u>Your Requested Files Are Here</u> 👇 \n\n</b>"
        else:
            cap = f"<b>🏷 ᴛɪᴛʟᴇ : <code>{search}</code>\n🧱 ᴛᴏᴛᴀʟ ꜰɪʟᴇꜱ : <code>{total_results}</code>\n⏰ ʀᴇsᴜʟᴛ ɪɴ : <code>{remaining_seconds} Sᴇᴄᴏɴᴅs</code>\n\n📝 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {message.from_user.mention}\n ᴘᴏᴡᴇʀᴇᴅ ʙʏ : ⚡ {message.chat.title or temp.B_LINK or 'MOVIESHUB'} \n\n🧾 <u>Your Requested Files Are Here</u> 👇 \n\n</b>"
